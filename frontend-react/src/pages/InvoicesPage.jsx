...
export default function InvoicesPage() {
  ...
  const openInvoiceEdit = async (invoiceId) => {
    setEditLoadingId(invoiceId)
    setFormError('')
    try {
      ...
      setForm({
        ...
        customerDateOfBirth: invoice.customerDateOfBirth || '',
      ...
      setNotice(getApiError(requestError, 'Unable to load invoice for editing'))
    } finally {
  ...
  const submit = async (event) => {
    ...
    const payload = { ...form, storeId: Number(form.storeId), items: form.items.map((item) => ({ productId: item.productId || null, itemName: item.itemName.trim(), quantity: Number(item.quantity), unitPrice: Number(item.unitPrice), discountPercentage: Number(item.discountPercentage || 0) })) }
    try {
      if (editingInvoiceId) {
      ...
      setOpen(false); setEditingInvoiceId(null); setForm(initialForm())
    } catch (requestError) { setFormError(getApiError(requestError, editingInvoiceId ? 'Unable to update invoice' : 'Unable to create invoice')) }
    finally { setSaving(false) }
  }

  return (
    <>
      <PageHeader title="Invoices" description="Create invoices and review transactions across your stores." actionLabel="New invoice" onAction={() => { setEditingInvoiceId(null); setForm(initialForm()); setFormError(''); setOpen(true) }} />
      <ModuleCard>
      ...
      <Dialog open={open} onClose={closeDialog} fullWidth maxWidth="md" slotProps={{ paper: { sx: { borderRadius: 3 } } }}>
        ...
        <DialogTitle sx={{ pr: 6 }}><Typography variant="h5">{editingInvoiceId ? 'Edit invoice' : 'Create invoice'}</Typography><Typography color="text.secondary" sx={{ fontSize: 13.5, mt: 0.5 }}>Enter customer and item details. Totals are calculated automatically.</Typography><IconButton type="button" onClick={closeDialog} sx={{ position: 'absolute', right: 12, top: 12 }}><CloseRoundedIcon /></IconButton></DialogTitle>
        <DialogContent dividers sx={{ pt: 2.5 }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: '1.1fr 1fr 1fr 1fr 1fr' }, gap: 2 }}>
            <TextField select required label="Store" value={form.storeId} onChange={(event) => setForm((current) => ({ ...current, storeId: event.target.value }))}>{stores.filter((store) => store.active || String(store.id) === String(form.storeId)).map((store) => <MenuItem key={store.id} value={store.id}>{store.name} ({store.storeCode})</MenuItem>)}</TextField>
        ...
        <DialogActions sx={{ p: 2 }}><Button type="button" onClick={closeDialog} color="inherit">Cancel</Button><Button type="submit" variant="contained" disabled={saving || !stores.length}>{saving ? (editingInvoiceId ? 'Saving…' : 'Creating…') : (editingInvoiceId ? 'Save changes' : 'Create invoice')}</Button></DialogActions>
        </Box>
      </Dialog>
